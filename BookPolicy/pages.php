...
?>
	...
	<script>
		...
		<!-- Added ID to aid in the JavaScript 
			 JP -->

		<a href="<?=confirm_slash(base_url()).confirm_slash($book->slug)?>new.edit" style="float:right;font-size:11px !important;" class="generic_button" id="createNewPageButton">Create new page</a>

		<!-- Create script to ensure that each book only has one article
			 If the requirements go change, a simple if...then statement
			 can be used to increase the functionality
			 JP -->

		<script>
			$("#createNewPageButton").click(function(event) {
				event.preventDefault();
				alert("Sorry, but policy dictates one article per book");
			});
		</script>
